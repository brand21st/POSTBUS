-- INSERT ... RETURNING on organizations requires a SELECT policy.
-- New owners are not members yet, so is_org_member() fails during create.

drop policy if exists orgs_select_creator on public.organizations;
create policy orgs_select_creator on public.organizations
