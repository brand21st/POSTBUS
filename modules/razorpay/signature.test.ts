...
import { verifyCheckoutSignature, verifyWebhookSignature } from "@/modules/razorpay/signature";

const secret = "whsec_test";

describe("razorpay signatures", () => {
  it("accepts a valid checkout signature and rejects a tampered one", () => {
    ...
    expect(
      verifyCheckoutSignature({ paymentId, subscriptionId, signature: "a".repeat(signature.length), secret })
    ).toBe(false);
  });
